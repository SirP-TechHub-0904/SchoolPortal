...
namespace SchoolPortal.Web.Areas.Account.Controllers
{
    [Authorize]
    public class XYZController : Controller
    {
        ...
        public XYZController(ApplicationUserManager userManager, ApplicationRoleManager roleManager, ApplicationSignInManager signInManager, UserManagerService userService)
        {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
                ...
                if (removePassword.Succeeded)
                {
                    ...
                    if (AddPassword.Succeeded)
                    {
                        ...
                        else
                        {
                            ...
                            return RedirectToAction("Login", "Account", new { area = "XYZ" });
                        }
                    }
                }
            }
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    ...
                    {
                        return RedirectToAction("UserLockout", new { userid = user.Id, area = "Account" });
                    }
        ...
        public async Task<ActionResult> ClearCatch()
        {
            IQueryable<Tracker> enrolment = from s in db.Trackers.AsNoTracking().OrderByDescending(x=>x.ActionDate)
                                            select s;
            var take1000 = await enrolment.Take(5000).AsNoTracking().ToListAsync();
            var xc = take1000.Count();
            if (xc == 5000)
            {
                foreach (var x in take1000.ToList())
                {
                    Tracker models = await db.Trackers.FindAsync(x.Id);
                    db.Trackers.Remove(models);
                }
                await db.SaveChangesAsync();
