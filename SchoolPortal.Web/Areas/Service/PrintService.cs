...
namespace SchoolPortal.Web.Areas.Service
{
    public class PrintService
    {
        ...
        #region print result
        public static PrintResultDto PrintResult(Int32? id, string batchid)
        {
            using (var db = new ApplicationDbContext())
            {
                ...
                var getuserenrollment = db.Enrollments.Include(p => p.ClassLevel).Include(x => x.ClassLevel.Subjects).FirstOrDefault(x => x.Id == id);
                ...
                var subjectlis = db.EnrolledSubjects.Include(x => x.Subject).Include(z => z.Enrollments).Where(d => d.EnrollmentId == id && d.IsOffered == true);
                var subjectlist = db.EnrolledSubjects.Include(x => x.Subject).Include(z => z.Enrollments).Where(d => d.EnrollmentId == id && d.IsOffered == true);
                ...
                //<td style="padding:3px;">@item.Subject.SubjectName</td>
                //           <td style="padding:3px;">@item.TestScore</td>
                //           <td style="padding:3px;">@item.ExamScore</td>
                //           <td style="padding:3px;">@item.TotalScore</td>
                //           <td style="padding:3px;">@item.HighestTotalScore</td>
                //           <td style="padding:3px;">@item.LowestTotalScore</td>
                //           <td style="padding:3px;">@item.Grade</td>
                //           <td style="padding:3px;">@item.Remark</td>

                var outputSubjectList = subjectlist.Select(c => new SubjectListPrintoutDto()
                {
                    Id = c.Id,
                    Subject = c.Subject,
                ...
                int subjectCount = db.EnrolledSubjects.Where(x => x.EnrollmentId == id && x.IsOffered == true).Count();
                int getuserenrollmentcount = db.Enrollments.Include(p => p.ClassLevel).Include(x => x.ClassLevel.Subjects).Where(x => x.Id == id).Count();
                ...
                if (subjectCount != 0)
                {
                    ...
                    var positions = db.Enrollments.Include(x => x.StudentProfile).Include(x => x.EnrolledSubjects).Where(x => x.SessionId == getuserenrollment.SessionId && x.ClassLevelId == getuserenrollment.ClassLevelId && x.AverageScore > 0).OrderByDescending(x => x.AverageScore).ThenBy(x => x.StudentProfile.StudentRegNumber);
                    try
                    {
                ...
                var recognitivedomain = new RecognitiveDomain();
                var recognitivedomain1 = db.RecognitiveDomains.FirstOrDefault(x => x.EnrolmentId == getuserenrollment.Id);
                ...
                var psychomotordomain1 = db.PsychomotorDomains.FirstOrDefault(x => x.EnrolmentId == getuserenrollment.Id);
                ...
                var affectivedomain1 = db.AffectiveDomains.FirstOrDefault(x => x.EnrolmentId == getuserenrollment.Id);
                ...
                // SchoolFees
                var schoolfee = db.SchoolFees.Where(x => x.SessionId == getenrollmentsession.Id).ToList();
                //var newsletter = new NewsLetter();
                var newsletter = db.NewsLetters.Where(x => x.SessionId == getenrollmentsession.Id).OrderByDescending(x => x.SessionId).Take(1).ToList();
                #endregion
                //check promoted or not
                var subjectMath = subjectlist.FirstOrDefault(x => x.Subject.SubjectName.Substring(0, 6).ToUpper() == "MATHEM");
                var subjectEng = subjectlist.FirstOrDefault(x => x.Subject.SubjectName.Substring(0, 7).ToUpper() == "ENGLISH");
                // user class
        ...
        /// <returns></returns>
        /// 
        public static decimal PaymentModeStatus(int id, int studentid, int seeid)
        {
            using (var db = new ApplicationDbContext())
            {
                ...
                if (finance != null)
                {
                    if (finance.PaymentTypeId == paymode.Id)
                    {
        ...
        public static decimal PaymentModeStatusInitial(int id, int studentid, int seeid)
        {
            using (var db = new ApplicationDbContext())
            {
                var enrol = db.Enrollments.FirstOrDefault(x => x.StudentProfileId == studentid && x.SessionId == seeid);
                var finance = db.FinanceInitializers.Where(x => x.IncomeId == id && x.EnrollmentId == enrol.Id).OrderByDescending(x => x.Id).FirstOrDefault();
                var paymode = db.Incomes.FirstOrDefault(x => x.Id == id);
                if (finance != null)
                {
                    if (finance.PaymentTypeId == paymode.Id)
        ...
        public static PrintResultDto PrintSingleResult(Int32? id)
        {
            using (var db = new ApplicationDbContext())
            {
                ...
                var subjectlis = db.EnrolledSubjects.Include(x => x.Subject).Include(z => z.Enrollments).Where(d => d.EnrollmentId == id && d.IsOffered == true);
                var subjectlist = db.EnrolledSubjects.Include(x => x.Subject).Include(z => z.Enrollments).Where(d => d.EnrollmentId == id && d.IsOffered == true);
                //var subjectlistRangeScore = db.EnrolledSubjects.Include(x => x.Subject).Include(z => z.Enrollments).Where(d => d.Enrollments.ClassLevelId == getuserenrollment.ClassLevelId && d.IsOffered == true).ToList();
                var total = db.EnrolledSubjects.Where(f => f.EnrollmentId == id).Sum(x => x.TotalScore);
                int subjectCount = db.EnrolledSubjects.Where(x => x.EnrollmentId == id && x.IsOffered == true).Count();
                int getuserenrollmentcount = db.Enrollments.Include(p => p.ClassLevel).Include(x => x.ClassLevel.Subjects).Where(x => x.Id == id).Count();
                ...
                if (subjectCount != 0)
                {
                    ...
                    //  var GETaa = db.BatchResults.Where(x => x.BatchId == batchid).OrderByDescending(x => x.AverageScore);
                    //   var useposl = GETaa.Where(x => x.AverageScore == getuserenrollment.AverageScore);
                    //var GETaa = db.Enrollments.Include(x => x.StudentProfile).Include(x => x.EnrolledSubjects).Where(x => x.SessionId == getuserenrollment.SessionId && x.ClassLevelId == getuserenrollment.ClassLevelId && x.AverageScore > 0).OrderByDescending(x => x.AverageScore).ThenBy(x => x.StudentProfile.StudentRegNumber);

                    //var usepositionforall = GETaa.Select(x => x.AverageScore);
                    //var usepositionfor = db.Enrollments.Include(x => x.StudentProfile).Include(x => x.EnrolledSubjects).Where(x => x.SessionId == getuserenrollment.SessionId && x.ClassLevelId == getuserenrollment.ClassLevelId && x.AverageScore > 0).OrderByDescending(x => x.AverageScore).ThenBy(x => x.StudentProfile.StudentRegNumber);



                    var GETaa = db.Enrollments.Include(x => x.StudentProfile).Include(x => x.EnrolledSubjects).OrderBy(x => x.StudentProfile.user.Surname).Include(x => x.StudentProfile).Where(s => s.ClassLevelId == getuserenrollment.ClassLevelId && s.SessionId == getuserenrollment.SessionId && s.EnrolledSubjects.Count() > 0);

                    var useposl = GETaa.Where(x => x.AverageScore == getuserenrollment.AverageScore);
                    var usepositionforall = useposl.Select(x => x.AverageScore);
                    var usepositionfor = db.Enrollments.OrderBy(x => x.StudentProfile.user.Surname).Include(x => x.StudentProfile).Where(s => s.ClassLevelId == getuserenrollment.ClassLevelId && s.SessionId == getuserenrollment.SessionId && s.EnrolledSubjects.Count() > 0 && s.AverageScore == getuserenrollment.AverageScore).OrderByDescending(x => x.AverageScore).ThenBy(x => x.StudentProfile.StudentRegNumber).ToArray();
                    var usepobbb = db.Enrollments.OrderBy(x => x.StudentProfile.user.Surname).Include(x => x.StudentProfile).Where(s => s.ClassLevelId == getuserenrollment.ClassLevelId && s.SessionId == getuserenrollment.SessionId && s.EnrolledSubjects.Count() > 0 && s.AverageScore == getuserenrollment.AverageScore).OrderByDescending(x => x.AverageScore).ThenBy(x => x.StudentProfile.StudentRegNumber).Select(x => x.StudentProfile.StudentRegNumber).ToArray();
        ...
        public static PrintResultDto Position12(Int32? id)
        {
            if (id == 5722)
            {
                var j = "yes";
            ...
            using (var db = new ApplicationDbContext())
            {
                ...
                string currentsessiomterm1;
                string currentsessiomterm2;
                var getuserenrollment = db.Enrollments.Include(x => x.EnrolledSubjects).Include(p => p.ClassLevel).Include(x => x.ClassLevel.Subjects).FirstOrDefault(x => x.Id == id);
                var getuserprofile = db.StudentProfiles.Include(c => c.user).FirstOrDefault(x => x.Id == getuserenrollment.StudentProfileId);
                var getenrollmentsession = db.Sessions.FirstOrDefault(x => x.Id == getuserenrollment.SessionId);
                currentsessiomterm = getenrollmentsession.Term.ToUpper() + " TERM " + getenrollmentsession.SessionYear + " " + "ACADEMIC SESSION";
                currentsessiomterm1 = getenrollmentsession.Term.ToUpper() + " TERM ";
                ...
                if (subjectCount != 0)
                {
                    ...
                    string mainAverage = averageScore.ToString("0.00");
                    var positions = db.Enrollments.Include(x => x.StudentProfile).Where(x => x.SessionId == getuserenrollment.SessionId && x.ClassLevelId == getuserenrollment.ClassLevelId && x.AverageScore > 0).OrderByDescending(x => x.AverageScore).ThenBy(x => x.StudentProfile.StudentRegNumber);
                    totalStudents = positions.Count();
                    int position = 0;
                    decimal? avg = 0;
                    try
                ...
                var grdsystem = db.GradingDetails.Where(x => x.GradingId == avb.Id);
                var SchoolAcct = db.SchoolAccounts.ToList();
                var recognitivedomain = db.RecognitiveDomains.FirstOrDefault(x => x.EnrolmentId == getuserenrollment.Id);
                var psychomotordomain = db.PsychomotorDomains.FirstOrDefault(x => x.EnrolmentId == getuserenrollment.Id);
                var affectivedomain = db.AffectiveDomains.FirstOrDefault(x => x.EnrolmentId == getuserenrollment.Id);
                // SchoolFees
                var schoolfee = db.SchoolFees.Where(x => x.SessionId == getenrollmentsession.Id).ToList();
        ...
        public static int MasterListPosition(Int32? id)
        {
            using (var db = new ApplicationDbContext())
            {
                ...
                if (subjectlist.Count() != 0)
                {
                    ...
                    var GETaa = db.Enrollments.Include(x => x.StudentProfile).OrderBy(x => x.StudentProfile.user.Surname).Include(x => x.StudentProfile).Where(s => s.ClassLevelId == getuserenrollment.ClassLevelId && s.SessionId == getuserenrollment.SessionId && s.EnrolledSubjects.Count() > 0);
        ...
        public static int CummulativeMasterListPosition(Int32? id)
        {
            using (var db = new ApplicationDbContext())
            {
                ...
                if (subjectlist.Count() != 0)
                {
                    ...
                    var usepositionfor = db.Enrollments.OrderBy(x => x.StudentProfile.user.Surname).Include(x => x.StudentProfile).Where(s => s.ClassLevelId == getuserenrollment.ClassLevelId && s.SessionId == getuserenrollment.SessionId && s.EnrolledSubjects.Count() > 0 && s.CummulativeAverageScore == getuserenrollment.CummulativeAverageScore).OrderByDescending(x => x.CummulativeAverageScore).ThenBy(x => x.StudentProfile.StudentRegNumber).ToArray();
                    var usepobbb = db.Enrollments.OrderBy(x => x.StudentProfile.user.Surname).Include(x => x.StudentProfile).Where(s => s.ClassLevelId == getuserenrollment.ClassLevelId && s.SessionId == getuserenrollment.SessionId && s.EnrolledSubjects.Count() > 0 && s.CummulativeAverageScore == getuserenrollment.CummulativeAverageScore).OrderByDescending(x => x.CummulativeAverageScore).ThenBy(x => x.StudentProfile.StudentRegNumber).Select(x => x.StudentProfile.StudentRegNumber).ToArray();
                    if (subjectlist.Count() != 0)
                    {
        ...
        public static PrintThirdTermDto PrintThirdTermResult(Int32? id, int sessionId)
        {
            using (var db = new ApplicationDbContext())
            {
                ...
                string currentsessiomterm1;
                string currentsessiomterm2;
                var getuserenrollment = db.Enrollments.Include(x => x.Session).Include(p => p.ClassLevel).Include(x => x.ClassLevel.Subjects).FirstOrDefault(x => x.Id == id);
                ...
                var output = new PrintThirdTermDto();
                string promote = "";
                //grading system in result =
                var avb = db.Gradings.Include(x => x.GradingDetails).FirstOrDefault(x => x.Name == getuserenrollment.ClassLevel.ClassName.Substring(0, 3));
                var grdsystem = db.GradingDetails.Where(x => x.GradingId == avb.Id);
                ...
                outloop: cumPosition = cummulativePosition;
                    ...
                    output = new PrintThirdTermDto
                    {
                        ...
                        Website = setting.WebsiteLink,
                        NewsLetter = newsletter,
                        SessionId = getenrollmentsession.Id,
                        TotalScore = totalsc,
                        OverallScore = overall,
                        PromotionStatus = promote,
        ...
        public static string AverageScoreSubject(int? id, int? StudentId, int? SessionId)
        {
            string subc = "";
            using (var db = new ApplicationDbContext())
            {
                try
                {
                    ...
                    var currentEnrollment = db.Enrollments.Include(x => x.ClassLevel).OrderByDescending(x => x.Id).FirstOrDefault(x => x.StudentProfileId == StudentId);
                    var currentYear = db.Sessions.FirstOrDefault(f => f.Id == SessionId);
                    string year = currentYear.SessionYear;
                    var getuserprofile = db.StudentProfiles.Include(c => c.user).FirstOrDefault(x => x.Id == getuserenrollment.StudentProfileId);
        ...
        public static string PromotionSubject(int? StudentId)
        {
            ...
            using (var db = new ApplicationDbContext())
            {
                try
                {
                    ...
                    var subjectss = db.EnrolledSubjects.Include(x => x.Subject).Where(x => x.EnrollmentId == enrolmnt.Id && x.IsOffered == true).ToList();
                    var subjectMath = subjectss.FirstOrDefault(x => x.Subject.SubjectName.Substring(0, 6).ToUpper() == "MATHEM");
                    var subjectEng = subjectss.FirstOrDefault(x => x.Subject.SubjectName.ToUpper().Contains("ENGLISH STUDIES") || x.Subject.SubjectName.ToUpper().Contains("ENGLISH LANGUAGE"));
                    var mathsidd = db.EnrolledSubjects.Include(x => x.Subject).FirstOrDefault(x => x.Id == subjectMath.Id);
                    var engidd = db.EnrolledSubjects.Include(x => x.Subject).FirstOrDefault(x => x.Id == subjectEng.Id);
                    ...
                    foreach (var a1 in subjectss)
                    {
                        ...
                        if (a1.Id == subjectMath.Id)
                        {
                            try
                            {
                                ...
                                //var firsttermsubject = db.EnrolledSubjects.FirstOrDefault(x => x.Id == mathsidd.Id && x.EnrollmentId == firstEnrollment.Id);
                                //var secondtermsubject = db.EnrolledSubjects.FirstOrDefault(x => x.Id == mathsidd.Id && x.EnrollmentId == secondEnrollment.Id);
                                //var thirdtermsubject = db.EnrolledSubjects.FirstOrDefault(x => x.Id == mathsidd.Id && x.EnrollmentId == thirdEnrollment.Id);
                                var secondtermsubject = secondEnrollment.EnrolledSubjects.FirstOrDefault(x => x.SubjectId == subjectMath.SubjectId && x.EnrollmentId == secondEnrollment.Id);
                                var thirdtermsubject = thirdEnrollment.EnrolledSubjects.FirstOrDefault(x => x.SubjectId == subjectMath.SubjectId && x.EnrollmentId == thirdEnrollment.Id);
                                var firsttermsubject = firstEnrollment.EnrolledSubjects.FirstOrDefault(x => x.SubjectId == subjectMath.SubjectId && x.EnrollmentId == firstEnrollment.Id);
                                decimal? Ave1 = 0.00m;
                                decimal? Ave2 = 0.00m;
                                decimal? Ave3 = 0.00m;
                                if (firsttermsubject != null)
                    ...
                    foreach (var a2 in subjectss)
                    {
                        ...
                        if (a2.Id == subjectEng.Id)
                        {
                            try
                            {
                                ...
                                var secondtermsubject = secondEnrollment.EnrolledSubjects.FirstOrDefault(x => x.SubjectId == subjectEng.SubjectId && x.EnrollmentId == secondEnrollment.Id);
                                var thirdtermsubject = thirdEnrollment.EnrolledSubjects.FirstOrDefault(x => x.SubjectId == subjectEng.SubjectId && x.EnrollmentId == thirdEnrollment.Id);
                                var firsttermsubject = firstEnrollment.EnrolledSubjects.FirstOrDefault(x => x.SubjectId == subjectEng.SubjectId && x.EnrollmentId == firstEnrollment.Id);
                                decimal? Ave1 = 0.00m;
                                decimal? Ave2 = 0.00m;
                                decimal? Ave3 = 0.00m;
                                if (firsttermsubject != null)
